...
<body>
	...
	<div class="cf">
		<div class="fl">
			<a class="btn" href="<?php echo U('MeetingProblem/add',array('pid'=>$pid));?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('MeetingProblem/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('MeetingProblem/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('MeetingProblem/changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
			
			<button class="btn copybtn" url="<?php echo U('MeetingProblem/problemcopy');?>" target-form="ids">复 制</button>
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
			<?php if($vo[pid] == 0): ?><a href="<?php echo U('MeetingProblem/index',array('pid'=>$vo[id]));?>"><?php echo ($vo["title"]); ?></a>
			<?php else: ?>
			<?php echo ($vo["title"]); endif; ?>
			</td>
			<td><?php echo (getmeetproblemtype($vo[type])); ?></td>
			<td><?php echo (modelfield($vo["hd_id"],"meeting","title")); ?></td>
			<td><?php echo ($vo["sort"]); ?></td>
			<td><?php echo (date('Y-m-d H:i',$vo["create_time"])); ?></td>
			<td><?php echo (get_status_title($vo["status"])); ?></td>
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('MeetingProblem/changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('MeetingProblem/changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
                <a href="<?php echo U('MeetingProblem/edit',array('pid'=>$pid,'id'=>$vo['id']));?>">编辑</a>
				<a href="<?php echo U('MeetingProblem/changeStatus?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
	...
	$(".copybtn").click(function(){
		...
		//$.post("<?php echo U('problemcopy');?>",{proarr:spCodesTemp},function(d){
