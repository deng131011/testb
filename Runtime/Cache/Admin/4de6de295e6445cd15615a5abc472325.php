...
<body>
	...
	<div class="main-title">
		...
		<h2 style="margin-left:20px;">参与人数：<span style="color: red;"><?php echo ($listsCount); ?></span></h2>
	</div>
	<div class="cf">
		<div class="fl">
            <!--<button class="btn ajax-post" url="<?php echo U('Signin/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>-->
            <button class="btn ajax-post confirm" url="<?php echo U('Joinperson/del',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $n = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><tr>
			...
			<td><?php echo (modelfield($vo["hd_id"],"gsmeeting","title")); ?></td>
			<td>
			     <?php if($vo[rander] == 1): ?><font style="color:red">已中奖</font>
				 <?php else: ?>
				    <font style="color:gray">未中奖</font><?php endif; ?>
			</td>
			<td><?php echo (date('Y-m-d H:i',$vo["create_time"])); ?></td>
			
			<td>
				<a href="<?php echo U('Joinperson/del?id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Joinperson/index');?>');
	</script>
