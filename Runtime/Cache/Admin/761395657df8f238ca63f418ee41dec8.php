...
<body>
	...
	<div class="cf">
		<div class="fl">
			<?php if($_GET['typeid'] != ''): ?><a class="btn" href="<?php echo U('Gsmeeting/add?typeid='.$_GET['typeid']);?>">新 增</a>
			<?php else: ?>
			<a class="btn" href="<?php echo U('Gsmeeting/add');?>">新 增</a><?php endif; ?>
            <button class="btn ajax-post" url="<?php echo U('Gsmeeting/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('Gsmeeting/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Gsmeeting/del');?>" target-form="ids">删 除</button>
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Gsmeeting/changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('Gsmeeting/changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<a href="/<?php echo ($vo[erweima]); ?>" target="_blank">二维码</a>
				
               <!-- <a href="<?php echo U('Meeting/edit',array('typeid'=>$typeid,'id'=>$vo['id']));?>">编辑</a>-->
				<a href="<?php echo U('Gsmeeting/del&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Gsmeeting/index');?>');
	</script>
