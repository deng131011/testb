...
<body>
	...
	<div class="cf">
		<div class="fl">
			<?php if($_GET['typeid'] != ''): ?><a class="btn" href="<?php echo U('Online/add?typeid='.$_GET['typeid']);?>">新 增</a>
			<?php else: ?>
			<a class="btn" href="<?php echo U('Online/add');?>">新 增</a><?php endif; ?>
            <button class="btn ajax-post" url="<?php echo U('Online/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('Online/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('Online/changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<form action="/Admin/Online/index" id="form1" method="get">
			<div class="sleft">
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><span><?php echo ($vo["view"]); ?>次</span></td>
			<td><?php echo (date('Y-m-d H:i',$vo["create_time"])); ?></td>
			<td><?php echo (get_status_title($vo["status"])); ?></td>
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Online/changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('Online/changeStatus?method=resumeUser&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
                <a href="<?php echo U('Online/edit',array('typeid'=>$typeid,'id'=>$vo['id']));?>">编辑</a>
				<a href="<?php echo U('Online/changeStatus?method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Online/index');?>');
	</script>
