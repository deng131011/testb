...
<body>
	...
	<div class="main-title">
		...
		<!--<h2 style="margin-left:20px;">未查看：<span style="color: red;">
		   <?php if($LabourCounts == null): ?>0<?php else: echo ($LabourCounts); endif; ?>
		</span></h2>
		<h2 style="margin-left:20px;">未回复：<span style="color: red;">
		<?php if($ansCounts == null): ?>0<?php else: echo ($ansCounts); endif; ?>
		</span></h2>-->
	</div>
	<div class="cf">
		<div class="fl">
           <!-- <button class="btn ajax-post" url="<?php echo U('Labour/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>-->
            <button class="btn ajax-post confirm" url="<?php echo U('Labour/del',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>
    ...
    <tbody id="infocontent">
		<?php if(!empty($list)): if(is_array($list)): $n = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><tr>
			...
			<td><?php echo ($n); ?> </td>
			<td><?php echo (modelfield($vo["cid"],"usermember","open_id")); ?></td>
			<td><?php echo (modelfield($vo["cid"],"usermember","nickname")); ?></td>
			<td>
               <?php if(modelField($vo[cid],'usermember','headimgurl') != ''): ?><a href='<?php echo (modelfield($vo["cid"],"usermember","headimgurl")); ?>' target="_blank"><img src="<?php echo (modelfield($vo["cid"],"usermember","headimgurl")); ?>" style="width:80px; height:50px;" /></a>
			    <?php else: ?>
			      暂无头像<?php endif; ?> 

			</td>
			<td><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></td>
			<td><?php echo onlineStatus($vo[id]);?></td>
			<td>
                <a href="<?php echo U('Labour/edit?cid='.$vo['cid']);?>">查看</a>
				<a href="<?php echo U('Labour/del?cid='.$vo['cid']);?>" class="confirm ajax-get">删除</a>
                </td>
	...
	<input type="hidden" id="status" value="<?php echo ($get['status']); ?>"> 
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Labour/index');?>');
	...
	function Reload(){
	    ...
	    $.post("<?php echo U('ajaxindexlist');?>",{type:'list',status:obj},function(d){
