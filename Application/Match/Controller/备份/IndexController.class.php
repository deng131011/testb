...
class IndexController extends HomeController {
	...
	public function forms(){
		if(IS_POST){
			...
			$nickname = session('match_nickname');
			if(empty($openid)||$openid==''){
				$openid = 'a'.rand(1000,9999).'axfH'.rand(1000,9999).'hkjxh';
			}
			...
			$where['typepid']  = $resu['id'];
			$info = M('match_person')->where($where)->find();
			if(empty($info)){
				$data['create_time'] = time();
				$data['typepid']     = $resu[id];
				$data['open_id']     = $openid;
				$data['nickname']    = $nickname;
				$data['company']     = $pt[company];
				$data['username']    = $pt[username];
				$data['mobile']      = $pt[mobile];
				$resuts = M('match_person')->add($data);
				if($resu){
					session('match_person_id',$resuts);
					$msg[status] = 1;
				}else{
					$msg[status] = 2;
					$msg[msg]    = '获取个人信息失败,请重新进入';
				}
				
			}else{
				session('match_person_id',$info[id]);
				$msg[status] = 1;
			}
			$this->ajaxReturn($msg);
			...
			$mys[open_id] = $openid;
			$mys[typepid] = $resu[id];
			$tth = M('match_person')->where($mys)->find();
				if(!empty($tth)){
					session('match_person_id',$tth['id']);
	...
	public function numlist(){
		...
		$isuid = M('match_person')->find($uid);
		if(empty($isuid)){
			$this->redirect('forms');exit;
		}
	...
	public function problem(){
		...
		if($typeid>0){
			...
			//echo M('match_numlist')->getLastSql();
			if(empty($numResu)){
				$data[jg_times] = 12;
	...
	public function addproblem(){
		...
		if($reter){
			...
			if($datarr['answer']==$sureanser){
				...
				$msg[status] = 4;
			    $msg[msg]    = '<p style="text-align:center;"><font style="color:red">回答错误 </font></p><p style="text-align:center;">正确答案为：'.$sureanser.'</p>';
			}
