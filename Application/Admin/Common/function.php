...
function onlineStatus($id){
	...
	//$where[cid] = $cid;
	//$where[pid] = 0;
	//$where[status] = array(array('gt',-1),array('lt',3));
	//$data = M('labour')->where($where)->select();
	
	$data = M('labour')->find($id);
	if($data['status']==3){
		return '<font style="color:#9505EF;">已回复</font>';
	}else{
...
function getMeetProblemType($type){
...
function meetQdstatus($id,$hd_id,$uid){
	

    $info = M('meeting_signin')->where('id='.$id)->find();

    
        if($info[dt_status]==1){
           if($hd_id<=16){
			    
                    if($info[win_status]==1){
					   return '<font style="color:red">已中奖</font>';
					}else{
						return '<font style="color:#b12be8">未中奖</font>';
					}
				
           }else{
                $whes[uid]   = array('eq',$uid);
                $whes[hd_id] = array('eq',$hd_id);
                $mmt = M('win_lists')->where($whes)->find();
...
function getOtherCompany($company_id,$qd_id){
	
	$data = M('meeting_company')->find($company_id);
	if($data[title]=='其他'){
		$info = M('meeting_signin')->where('id='.$qd_id)->find();
		return $info['othercompany']; 
	}else{
		return $data[title];
	}
