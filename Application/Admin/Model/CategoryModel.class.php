...
class CategoryModel extends Model{
    ...
    public function getTree($id = 0, $field = true){
		...
		if(UID!=1){
			 $listers= M('auth_extend')->where("b.uid=".UID)->field('bhy_auth_extend.extend_id')->join('bhy_auth_group_access as b on bhy_auth_extend.group_id=b.group_id')->select();
			
            foreach($listers as $k=>$v){
                $str[] =$v[extend_id];
            }
            $strs = implode(',',$str);
          
            $map["id"] = array('in',$strs);
		}
		 /* 获取所有分类 */
