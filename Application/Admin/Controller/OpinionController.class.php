...
class OpinionController extends AdminController {
	...
	public function index(){
        //未查看
		$OpinionCounts = M('Opinion')->where('status=1 and pid=0')->count();
		$this->assign('OpinionCounts',$OpinionCounts);
		
		 //已查看、未回复
		$ansCounts = M('Opinion')->where('status=2 or status=1 and pid=0')->count();
		...
		//分页
		$listsCount = M('Opinion')->where($where)->count();
		$Page       = new \Think\Page($listsCount,15);// 实例化分页类 传入总记录数和每页显示的记录数(25)
		$show       = $Page->show();// 分页显示输出
		
		$lists = M('Opinion')->where($where)->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function edit(){
            if(IS_POST){
				...
				$result = D('Opinion')->add($_POST);
                if($result){
					$mdrt = M('Opinion')->where('id='.$pid)->save(array('status'=>3));
					if($mdrt){
				...
				//已查看
				D('Opinion')->where($map)->save(array('status'=>2));
				
				$editdata = D('Opinion')->where($map)->find();
   ...
   //删除
	public function del(){
		...
		$delResult = M('Opinion')->where($where)->delete();
		$delResults = M('Opinion')->where($map)->delete();
		if($delResult){
