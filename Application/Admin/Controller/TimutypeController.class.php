 ...
 */
class TimutypeController extends AdminController {
    ...
    public function index(){
        $tree = D('Timutype')->getTree(0,'id,title,sort,pid,status');
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
			...
			if($pid){
				...
				$protype = M('Timutype')->find($pid);
				
				$bre['id'] = array('in',$protype['china']);
    ...
    public function remove(){
        ...
        $child = M('Timutype')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
    ...
    public function operate($type = 'move'){
        ...
        $list = M('Timutype')->where($map)->field('id,title')->select();
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('Timutype')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
	...
	public function batchadd(){
		...
		if(IS_POST){
			...
			$pretitle = explode(',',$_POST['title']);
			
			for($i = 0; $i < count($pretitle); $i++){
				
				$data[$i]['title'] = $pretitle[$i];
				$data[$i]['create_time'] = time();
				$data[$i]['update_time'] = time();
				$data[$i]['pid'] = $_POST['pid'];
				
			}
			$result = M('Timutype')->addAll($data);
