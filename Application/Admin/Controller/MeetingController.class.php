...
class MeetingController extends AdminController {
	...
	public function add(){
       ...
       if(IS_POST){
           ...
           if($meet->create($_POST)){
				...
				$winresu = M('win_lists')->addAll($winarr);
				
		        if($resu && $winresu){
		       	   //生成存入二维码
		       	   header("Content-type: text/html; charset=utf-8"); 
                   $path    = 'Uploads/Picture/erweima/';
                   $imgname = date('YmdHis');
                   $url     = C('WEB_SITE_URL').'/home/meeting/problem?hd_id='.$resu;
				   
		           $this->qrcode($url,$path,$imgname);
                   //存入二维码路径
                   $arre['erweima'] = $path.$imgname.'.png';
                   $ert = M('meeting')->where('id='.$resu)->save($arre);
	...
	public function del(){
		...
		if($id>0){
			...
			$resu = M('meeting')->where($where)->delete();
			if($resu){
    ...
    public function meetingId(){
		...
		$result = M('meeting')->where('meeting_id='.$meetid)->find();
		if(!empty($result)){
             $this->meetingId();
		}else{
			return $meetid;
