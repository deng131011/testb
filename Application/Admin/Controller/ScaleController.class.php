...
class ScaleController extends AdminController {
    ...
    public function index(){
        $tree = D('Scale')->getTree(0,'id,title,sort,pid,status');
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
			...
			$brand = M('Scale')->where($bre)->select();
    ...
    public function remove(){
        ...
        $child = M('Scale')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
    ...
    public function operate($type = 'move'){
        ...
        $list = M('Scale')->where($map)->field('id,title')->select();
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('Scale')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
